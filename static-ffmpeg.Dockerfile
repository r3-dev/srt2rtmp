...
FROM oven/bun:alpine AS final1
...
FROM oven/bun:alpine AS final2
COPY --from=final1 / /

FROM final2 as final3
...
FROM alpine:3.19.1 AS bun-builder2
RUN apk add --update curl bash
RUN curl -fsSL https://bun.sh/install | bash
ENV BUN_INSTALL="$HOME/.bun"
ENV PATH="${BUN_INSTALL}/bin:$PATH"
RUN bash bun --version
WORKDIR /app
COPY bun.lockb .
COPY package.json .
RUN bun install --frozen-lockfile
